    ...
    _setCalendarData: function (year, month) {
      ...
      for (let i = 0; i < empty_days_count; i++) {
        empty_days.push({
          ...
          date: `${prev_year}-${
            prev_month < 10 ? `0${prev_month}` : prev_month
          }`,
        });
      }
      ...
      for (let i = 0; i < 6 - last_date; i++) {
        empty_days_last.push({
          ...
          background: "transparent",
          date: `${next_year}-${
            next_month < 10 ? `0${next_month}` : next_month
          }`,
        });
      }
      ...
      if (this.data.showMoreDays) {
        ...
        if (index != -1) {
          ...
          for (let i = 1; i <= count; i++) {
            ...
            days[index + i - 1].date = `${days[index + i - 1].date}-${
              i < 10 ? `0${i}` : i
            }`;
        ...
        if (index != -1) {
          ...
          for (let i = 0; i <= index; i++) {
            ...
            days[i].date = `${days[i].date}-${
              days[i].day < 10 ? `0${days[i].day}` : days[i].day
            }`;
      ...
      for (let i = 0; i < this.data.days_color.length; i++) {
        ...
        for (let j = 0; j < days.length; j++) {
          // console.log(days[j].date)
          // console.log(item.date)
          // if(item.date=='2020-08-31'){
          // console.log(days[j].day);
          // }
          if (days[j].date == item.date) {
            // console.log(item.date)
            if (item.selectedColor) {
              days[j].selectedColor = item.selectedColor;
            }
            ...
            if (item.otherColor) {
              ...
              days[j].otherColor = item.otherColor;
            }
            if (item.badgeColor) {
              //附加角标颜色

              days[j].badgeColor = item.badgeColor;
            }
          }
        }
      }
